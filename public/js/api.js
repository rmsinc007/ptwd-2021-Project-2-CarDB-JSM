...
const getInfectionButton = document.querySelector('#get-infection-button');
...
//infections button
getInfectionButton.addEventListener( "click", () => {
  console.log("click");
  const infectArticle = infectInput.value;
  const options = {
    method: 'GET',
    url: `https://vaccovid-coronavirus-vaccine-and-treatment-tracker.p.rapidapi.com/api/news/get-coronavirus-news/0`,
    headers: {
  ...
  axios.request(options).then(function (response) {
    console.log(response.data.infect);
    // console.log(response[0]);
    const { title, link } = response.data.infect[0];
    // [...response.data].forEach((title,index) => {
    //   response.data.news[index].title
    //   console.log(title)
    // })
    // const title = response.data.news[0].title;
    response.data.infect.map((eachInfect)=>{
      let row = document.createElement('div')
      row.innerHTML=`<h3 id="article-name-h3"> ${eachInfect.title} </h3>

       <a href=${eachInfect.link} target="_blank" id="link">
           <button>Click to read</button>
      </a>`
      newsDiv.appendChild(row);
    })
